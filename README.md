...
> A Kubernetes capacity management system: a **validating admission webhook** that
> enforces a per-cluster CPU/RAM budget (fail-closed by design), and an optional
> **multi-cluster capacity equalizer** that optimizes budgets across a fleet.
> CI: GitHub Actions · License: Apache-2.0 · Kubernetes: 1.34–1.36 (N-2 support window)
...
src/equalizer/                  # multi-cluster capacity equalizer (spec-013): separate binary
├── mod.rs                      # module facade
├── crd.rs                      # EqualizerConfig CRD (targets + budget targets + status)
├── algorithm.rs                # pure equalization fn (overflow distribution in absolute units)
├── reconcile.rs                # read targets → compute budgets → patch overrides → write status
└── cluster_client.rs           # kube::Client from Secret-mounted kubeconfig (multi-cluster)
src/bin/capacity-equalizer/     # equalizer binary entry point (rustls provider + 10s runtime loop)
└── main.rs
