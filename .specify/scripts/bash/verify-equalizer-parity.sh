#!/usr/bin/env bash
# verify-equalizer-parity.sh — Kustomize↔raw parity for the equalizer bundle.
# spec-015, task T005 (Constitution Principle VIII: write the check first).
#
# Renders `kustomize build deploy/kustomize/equalizer` and compares the resource
# set against the pre-migration raw manifests (deploy/equalizer/{crds,deployment,
# rbac}.yaml), asserting field-level equivalence on EVERY field. The ONLY
# permitted difference is the container `image:` field
# (ERW_EQUALIZER_IMAGE_PLACEHOLDER in the raw → resolved by the kustomization
# `images:` directive in the bundle). Exits non-zero on any mismatch.
#
# The example-config.yaml is deliberately NOT compared: it is documentation,
# not listed in kustomization.yaml `resources:`, so kustomize does not render it
# (research R10). Its parity vs the raw equalizer-config.example.yaml is covered
# by the verbatim copy in the bundle.
set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
repo_root="$(cd "$script_dir/../../.." && pwd)"

bundle="$repo_root/deploy/kustomize/equalizer"
raw_files=(
  "$repo_root/deploy/equalizer/crds.yaml"
  "$repo_root/deploy/equalizer/deployment.yaml"
  "$repo_root/deploy/equalizer/rbac.yaml"
)
component="equalizer"
